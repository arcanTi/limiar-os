...
const DICE = (function() {
    ...
    var vars = { //todo: make these configurable on init
        ...
        use_sound: false, //set to false to mute the dice clatter entirely
        max_sound_hits: 3 //cap overlapping clatter sounds per throw (was 1 per die)
    ...
    that.dice_box = function(container) {
        ...
        this.world.solver.iterations = 10; //lowered from 16: cheaper solve, still stable for dice
    ...
    function throw_dices(box, vector, boost, dist, before_roll, after_roll) {
        ...
        // Play a small, fixed number of overlapping clatter sounds instead of
        // one per die. Spawning ~10 sounds per throw was a major source of the
        // audio lag when many dice rolled at once.
        let hits = Math.min(vectors.length, vars.max_sound_hits);
        for (let i = 0; i < hits; i++) {
            playSound(box.container, 0.4 + (i / Math.max(1, hits)) * 0.5);
        }
    ...
    that.dice_box.prototype.__animate = function(threadid) {
        ...
        if (vars.use_adapvite_timestep) {
            // Cap catch-up substeps: a single slow frame (common when many dice
            // collide) must not trigger a long chain of physics steps that makes
            // the next frame even slower — that feedback loop is what stutters.
            var steps = 0;
            while (time_diff > vars.frame_rate * 1.1 && steps < 4) {
                this.world.step(vars.frame_rate);
                time_diff -= vars.frame_rate;
                steps++;
            }
            // Clamp any leftover time so the final step stays small and stable.
            this.world.step(Math.min(time_diff, vars.frame_rate));
        }
        else {
            this.world.step(vars.frame_rate);
        }
    ...
    // Reusable, preloaded pool of audio elements. The original implementation
    // created a fresh <audio> node (and re-fetched/decoded the mp3) for every
    // die on every throw, which stuttered badly with many dice. We instead
    // preload a tiny pool once and just rewind+play a free element.
    var soundPool = null;
    var soundPoolIndex = 0;
    function playSound(outerContainer, soundVolume) {
        if (soundVolume === 0 || vars.use_sound === false) return;
        if (typeof Audio === 'undefined') return;
        if (!soundPool) {
            soundPool = [];
            for (var i = 0; i < 4; i++) {
                var a = new Audio('vendor/sarah-dice/assets/nc93322.mp3');
                a.preload = 'auto';
                soundPool.push(a);
            }
        }
        var audio = soundPool[soundPoolIndex];
        soundPoolIndex = (soundPoolIndex + 1) % soundPool.length;
        try {
            audio.currentTime = 0;
            audio.volume = Math.max(0, Math.min(1, soundVolume));
            var playback = audio.play();
            if (playback && playback.catch) playback.catch(function() {});
        } catch (e) { /* ignore autoplay/seek races */ }
    }
