# docs/MAP-ENGINE.md — Auditoria real do motor de mapa + rota para nivel Foundry

Criado 2026-07-17; documento histórico. Os caminhos atuais são
`frontend/src/pages/campaign-map.js`, `backend/routers/campaign_maps.py` e
`backend/repositories/campaign_maps.py`. Fonte original: auditoria AO VIVO no
browser (servidor local, login GM `mestre`, campanha `ts-verify-campaign`),
cruzada com leitura do codigo. Nada aqui e opiniao de plano: cada
...
| Sincronismo (F7) | `GET /updates?since=N` consulta o event log PostgreSQL; triggers gravam `map/chat/combat/roster` na mesma transacao e todos os workers observam o mesmo stream |
