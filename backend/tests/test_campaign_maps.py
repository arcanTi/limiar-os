...
def test_upsert_token_can_link_an_existing_token_to_a_character(db_path):
    """Re-saving an existing token must preserve a new character link."""
    campaign_id = "camp-1"
...
def test_map_state_combat_block_reflects_active_turn(db_path):
    """map_state must expose enough for the map to highlight whose turn
...
def test_delete_token_gcs_its_reveals(db_path):
    """A reveal must not outlive the token that earned it forever:
...
# Token HUD ammo reads `magazine` and `currentAmmo` persisted by the sheet or
# cockpit after gear normalization. These tests exercise the token payload's
# read side, following the same pattern as HP, injuries, and status effects.
...
# --- Template `untilResolved` lifecycle ------------------------------------
