...
def _fk_users(db_path):
    """Seed users referenced by literal sessions in this module.

    Campaigns remain owned by individual tests because one assertion compares
    the exact set returned by `list_campaigns_for`.
    """
    seed_session_users()
...
def test_campaign_map_ping_lifecycle(db_conn):
    ...
    # Pings outside the visibility window disappear from map_state and are
    # pruned from the database on the next insert.
    from backend.db import db

    with db() as conn:
        conn.execute(
            "UPDATE campaign_map_pings SET created_at = CURRENT_TIMESTAMP - INTERVAL '120 seconds' WHERE id = %s",
            (ping["id"],),
        )
