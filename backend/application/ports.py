...
class RecordRepository(Protocol):
    ...
    def list(self, kind: str, campaign_id: str | None = None) -> list[Record]: ...
    def get(self, kind: str, record_id: str) -> Record | None: ...
    def upsert(self, kind: str, payload: Record) -> Record: ...
    def upsert_revisioned(
        self, kind: str, payload: Record, expected_revision: int | None
    ) -> Record: ...
...
class CampaignAccessRepository(Protocol):
    ...
    def is_campaign_owner(self, campaign_id: str, session: dict[str, str]) -> bool: ...
    def list_campaigns_for(self, session: dict[str, str]) -> list[Record]: ...
    def delegated_character_ids(self, username: str) -> list[str]: ...
