...
describe('map gridRender: adaptiveGridStyle', () => {
  ...
  it('never exceeds the alpha ceiling even at max darkness and min zoom', () => {
    ...
    expect(style.alpha).toBeLessThanOrEqual(0.4);
  });

  it('never drops below the floor even at zero darkness and high zoom', () => {
    const style = adaptiveGridStyle(4, 0);
    expect(style.alpha).toBeGreaterThanOrEqual(0.08);
  });

  it('switches line tint from gray to lighter gray past the dark-scene threshold', () => {
    expect(adaptiveGridStyle(1, 0.1).colorRgb).toBe('150,150,150');
    expect(adaptiveGridStyle(1, 0.6).colorRgb).toBe('210,210,210');
  });
