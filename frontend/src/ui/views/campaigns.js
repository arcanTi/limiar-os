...
export function mountCampaignsOverlay({ api, documentRef = globalThis.document, activeCampaignId = '' } = {}) {
  ...
  controller.render();
  handlers.refresh();
  // Campaign changes arrive through the app's unified campaign sync channel.
  // The overlay also refreshes whenever it is opened; polling it while closed
  // previously generated up to five authenticated requests every 10 seconds.
