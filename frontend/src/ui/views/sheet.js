...
export function sheetRenderVals(state = {}, deps = {}) {
  ...
  // Read-only skill filtering narrows the full CPR catalog without changing the
  // character or its persisted representation.
  const skillQuery = String(S.skillQuery || '');
  const skillsOnlyTrained = !!S.skillsOnlyTrained;
  const visibleSkillRows = filterSkills(skillRows, { query: skillQuery, onlyTrained: skillsOnlyTrained });
  const skillFilterSummary = summarizeSkillFilter(skillRows, visibleSkillRows, {
    query: skillQuery,
    onlyTrained: skillsOnlyTrained,
  });

  const skillRowColumns = splitIntoColumns(visibleSkillRows);
  ...
  // Full page (SYS.01): Chrome is a standing right-column aside, not a tab.
  const sheetTabs = makeSheetTabs([
    { key: 'core', label: 'CORE' },
  ...
  // Lateral quick-access drawer: narrow single column, Chrome stays a tab
  // like the rest (no room for a standing aside at 560px).
  const sheetDrawerTabs = makeSheetTabs([
    { key: 'core', label: 'CORE' },
    { key: 'skills', label: 'SKILLS' },
    { key: 'conditions', label: 'CONDICOES' },
    { key: 'ip', label: 'IP' },
  ...
  return {
    ...
    // Two distinct hidden <input> ids: the full page and the lateral drawer
    // can both be mounted at once (avatar button stays reachable from the
    // full page's header), so they can't share one DOM id.
    triggerPlayerPortraitUpload: () => deps.triggerFileInput('player-portrait-upload'),
    triggerPlayerPortraitUploadDrawer: () => deps.triggerFileInput('player-portrait-upload-drawer'),
    ...
    skillQuery,
    skillsOnlyTrained,
    skillFilterActive: skillFilterSummary.filtering,
    skillFilterEmpty: skillFilterSummary.empty,
    skillFilterCount: `${skillFilterSummary.visible}/${skillFilterSummary.total}`,
    skillTrainedCount: skillFilterSummary.trained,
    skillOnlyTrainedStyle: 'lm-skill-filter-toggle' + (skillsOnlyTrained ? ' lm-skill-filter-toggle--on' : ''),
    onSkillQuery: (e) => deps.setState({ skillQuery: e.target.value }),
    toggleSkillsOnlyTrained: () => deps.setState({ skillsOnlyTrained: !skillsOnlyTrained }),
    clearSkillFilter: () => deps.setState({ skillQuery: '', skillsOnlyTrained: false }),
...
export function sheetHandlers(component) {
  ...
  async function saveSheetDraft() {
    ...
    const savedRaw = await component.app().persistCharacter.save(character, { player: playerSave });
