...
import { setI18n, i18nTranslations } from './i18n.ts';
import {
...
class Component extends DCLogic {
  state = {
    ...
    mapAttackContexts: {},
    // One-shot handoff from a resolved area template to the combat cockpit.
    ...
    // Situational attack-roll toggles. Global like tarotContext; the
    ...
    combatTargets: {},
    // Evasion prompts: pendingEvasion tracks the attacker's own
  ...
  componentDidMount() {
    ...
    this.refreshChat();
    // A single long-poll per active campaign covers chat,
    // combat/roster, so the only interval left is a 60s safety net for when
    // there's no active campaign yet or the long-poll degrades (matches the
    // map channel's own fallback poll).
    this.startCampaignSync();
    // Safety refresh is inactive while the campaign long-poll is healthy. It
    // only protects sessions without a campaign or a sync channel that has
    // failed for more than a minute.
    this._safety = setInterval(() => {
      const syncHealthy = this._syncLastSuccessAt && Date.now() - this._syncLastSuccessAt < 60000;
      if (!this.state.activeCampaignId || !syncHealthy) {
        this.refreshChat();
        this.refreshRoster();
      }
    }, 60000);
  ...
  async startCampaignSync() {
    ...
    while (!this._syncStopped) {
      ...
      try {
        ...
        this._syncLastSuccessAt = Date.now();
        this._syncRetryMs = 1000;
        ...
        if (!this._syncStopped) {
          const jitter = Math.floor(Math.random() * Math.max(1, this._syncRetryMs * 0.2));
          await this._syncDelay(this._syncRetryMs + jitter);
          this._syncRetryMs = Math.min(60000, this._syncRetryMs * 2);
        }
      } finally {
  ...
  async consumeMapAttackIntent() {
    ...
    try {
      ...
      clearMapAttackIntent(window.sessionStorage);
      // The map already knows darkness and line of sight, so
  ...
  // Handle "open sheet" and "open cockpit" from the map's token context menu.
  ...
  async consumeMapFocusIntent() {
    ...
    this.setState({ sheetOpen: true, sheetEditing: false, sheetCreating: false, sheetDraft: null, sheetTab: 'core' });
  }

  // Complete the area-template handoff by hydrating the intent the
  ...
  commitRoll(opts, faces, total, detail) {
    ...
    if (!this.state.gm) {
      ...
      // Tag evasion-prompt replies so the attacker's client can consume
 ...
 * special/deathSave atoms pass through resolved intact for downstream consumers.
 *
