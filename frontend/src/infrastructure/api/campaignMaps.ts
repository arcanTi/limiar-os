...
import type { CampaignMapState, CampaignUpdate } from './contracts.ts';
...
export function createCampaignMapsApi(request: HttpRequest, waitForCampaignEvent?: CampaignEventWaiter | null) {
  return {
    get: async (campaignId: string): Promise<CampaignMapState> => request(mapPath(campaignId)),
    waitForUpdate: async (campaignId: string, since: number, signal?: AbortSignal): Promise<CampaignUpdate> =>
      waitForCampaignEvent
        ...
        : request(mapPath(campaignId, '/updates?since=' + encodeURIComponent(String(since))), { signal }),
    ...
    resolveTemplate: async (campaignId: string, payload: Record<string, unknown>): Promise<unknown> => request(mapPath(campaignId, '/template/resolve'), { method: 'POST', body: JSON.stringify(payload || {}) }),
    ...
    saveProp: async (campaignId: string, payload: Record<string, unknown>): Promise<unknown> => request(mapPath(campaignId, '/prop'), { method: 'POST', body: JSON.stringify(payload || {}) }),
    deleteProp: async (campaignId: string, payload: Record<string, unknown>): Promise<unknown> => request(mapPath(campaignId, '/prop/delete'), { method: 'POST', body: JSON.stringify(payload || {}) }),
    damageProp: async (campaignId: string, payload: Record<string, unknown>): Promise<unknown> => request(mapPath(campaignId, '/prop/damage'), { method: 'POST', body: JSON.stringify(payload || {}) }),
