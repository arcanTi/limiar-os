...
// ---------- system badges ----------
// The shared system catalog keeps login and onboarding badges consistent.
...
function goToApp(campaignId, options = {}) {
  ...
  // `join` carries the user's intent to enter a campaign they do not yet belong
  // to, allowing the application to select the correct onboarding path.
  if (options.join) params.set('join', '1');
...
// Include campaigns the backend marks as joinable but which are not memberships
// yet, such as public tables and pending invitations.
...
function renderCampaignCard(c, user, opts) {
  ...
  const cta = joinable
    ...
    : 'Entrar';
  const readinessTag = `<span class="system-tag implementation-${meta.implementation}">Implementado · ${implementationLabel(c.system)}</span>`;
  return `
...
// Joinable cards carry the campaign id plus `join=1`. The application can then
// create or select a character without losing the campaign the user chose.
els.joinableList.addEventListener('click', (event) => {
  const btn = event.target.closest('[data-campaign-id]');
  if (!btn) return;
  goToApp(btn.getAttribute('data-campaign-id'), { join: true });
});
els.continueWithoutCampaign.onclick = () => goToApp('');

function updateSystemAvailability() {
  const system = els.newCampaignSystem.value;
  const implementation = systemMeta(system).implementation;
  els.systemAvailability.textContent = `Implementado · ${implementationLabel(system)}`;
