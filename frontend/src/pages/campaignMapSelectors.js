// Pure map-state selectors with no canvas or DOM writes. They only read the
// page's `state` object, query arguments, and domain helpers. Every function takes `state`
...
// `individual` exploration mode means live vision is not pooled across the
...
// "Measure and use in attack" shortcut. The menu and drag gesture share the
// same validated path.
