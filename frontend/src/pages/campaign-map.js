import '../styles/map/base.css';
import '../styles/map/themes/cyberpunk.css';
...
// Use application dialogs instead of window.prompt()/confirm(), with a
...
// The first automatic fitView() call
...
// While a token is being dragged, show a ghost
...
// Draw untreated critical injuries and active status effects as badges
// as small dots on the token's lower arc. Server already gated what arrived
// according to the resourceVisibility contract; this only renders what arrives.
...
// The ammo HUD is a small pill at the token's lower-right,
...
// The token HUD is anchored under the selected token, with inline HP and
...
// pixels, an explicit rendering-quality tradeoff),
...
// Destructible cover follows the same shape and tool pattern as walls
...
// The line-of-sight advisory is supplementary and never blocks the action; the real
...
// The "Measure and use in attack" shortcut uses the same validated path,
...
// sessionStorage envelope (mapFocusIntent), with the same discipline as attack
...
// Reloading from the map uses the same persistence path as combat.js reloadWeapon/
...
// here resets currentAmmo to magazine on the
...
// Dragging a template from its origin sets direction and distance (release ==
...
// Resolving an area lists tokens whose cell is inside templateCells()
// and inside this session's visibility (tokenVisibleNow), the same rule attack
...
// Double-click sends a ping to every client. A deterministic username color
// keeps it consistent across clients. Entries remain in pingAnims beyond the
// server window to prevent replaying the same animation after each refresh;
// only the first PING_ANIM_MS are rendered.
...
// Effective MOVE for the selected token. Character-linked tokens read the
// already-aggregated effectiveMoveStat from the authorized character record;
// unlinked tokens use their manual `move` value. Results are cached by
// characterId to avoid fetching on every redraw.
...
// Paint or erase difficult terrain one cell per drag. pointer.terrainTouched
// prevents repeated toggles while crossing the same cell. Each toggle is a
// backend read-modify-write operation, serialized through terrainQueue to
...
// Smooth zoom sets a target and interpolates toward it every frame
